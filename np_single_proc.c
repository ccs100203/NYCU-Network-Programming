...
void execCmd(char *cmd_token, char *cmd_rest, struct cmd_arg cmd_arg)
{
    ...
    if (cmd_arg.isPipe) {
        ...
        if (!pipe_arr[currSock][cmd_arg.numPipeLen].isValid) {
            debug("Num/ERR Pipe len: %ld\n", cmd_arg.numPipeLen);
            if (pipe(pipefd_rhs) == -1) {
                perror("pipe rhs error");
                exit(EXIT_FAILURE);
            }
            pipe_arr[currSock][cmd_arg.numPipeLen].isValid = true;
            pipe_arr[currSock][cmd_arg.numPipeLen].pipefd[0] = pipefd_rhs[0];
            pipe_arr[currSock][cmd_arg.numPipeLen].pipefd[1] = pipefd_rhs[1];
            isNewPipe = true;
        }
    }
    ...
    case 0: /* child */
        ...
        /* I/O Processing */
        /* replace STDIN */
        if (pipe_arr[currSock][0].isValid) {
            debug("replace STDIN\n");
            dup2(pipe_arr[currSock][0].pipefd[0], STDIN_FILENO);
            close(pipe_arr[currSock][0].pipefd[0]);
            close(pipe_arr[currSock][0].pipefd[1]);
        }
        ...
        if (cmd_arg.isPipe) {
            ...
            dup2(pipe_arr[currSock][cmd_arg.numPipeLen].pipefd[1], STDOUT_FILENO);
            close(pipe_arr[currSock][cmd_arg.numPipeLen].pipefd[0]);
            close(pipe_arr[currSock][cmd_arg.numPipeLen].pipefd[1]);
        } else if (cmd_arg.isErrPipe) {
            debug("child cmd_arg.isErrPipe\n");
            dup2(pipe_arr[currSock][cmd_arg.numPipeLen].pipefd[1], STDOUT_FILENO);
            dup2(pipe_arr[currSock][cmd_arg.numPipeLen].pipefd[1], STDERR_FILENO);
            close(pipe_arr[currSock][cmd_arg.numPipeLen].pipefd[0]);
            close(pipe_arr[currSock][cmd_arg.numPipeLen].pipefd[1]);
        ...
        for (int i = 1; i < 1002; i++) {
            if (pipe_arr[currSock][i].isValid) {  //  && (i != cmd_arg.numPipeLen)
                close(pipe_arr[currSock][i].pipefd[0]);
                close(pipe_arr[currSock][i].pipefd[1]);
            }
        }
    ...
    default: /* parent */
        ...
        if (pipe_arr[currSock][0].isValid) {
            ...
            close(pipe_arr[currSock][0].pipefd[0]);
            close(pipe_arr[currSock][0].pipefd[1]);
            pipe_arr[currSock][0].isValid = false;
        }
        /* normal pipe, add a pipefd to array */
        if (cmd_arg.isPipe) {
            debug("parent cmd_arg.isPipe\n");
            pipe_arr[currSock][0].isValid = true;
            pipe_arr[currSock][0].pipefd[0] = pipefd_rhs[0];
            pipe_arr[currSock][0].pipefd[1] = pipefd_rhs[1];
        }
    ...
    if (isForkErr) {
        ...
        if (isNewPipe) {
            ...
            if (cmd_arg.isNumPipe || cmd_arg.isErrPipe) {
                close(pipefd_rhs[0]);
                close(pipefd_rhs[1]);
                pipe_arr[currSock][cmd_arg.numPipeLen].isValid = false;
            }
        }
        /* redo this function */
